...
function FrontendCodeContent() {
  const router = useRouter()
  const [frontendCode, setFrontendCode] = useState<CodeFile[]>([])
  ...
  useEffect(() => {
    ...
    if (!artifactData || !artifactData.frontendCode) {
      router.push("/workflow")
      return
    }

    try {
      setFrontendCode(artifactData.frontendCode || [])
      ...
      console.error("Failed to parse frontend code data:", error)
      router.push("/workflow")
    }
  }, [router])
  ...
  const handleApprove = async () => {
    try {
      ...
      console.error("Failed to approve frontend code:", error)
    }
  }
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch("/api/submit-artifact-feedback", {
        ...
        body: JSON.stringify({
          artifactType: "frontend-code",
          artifactData: frontendCode,
          feedback: "User rejected this frontend code. Please provide detailed critique.",
      ...
      if (response.ok) {
        ...
        if (data.updatedArtifact) {
          setFrontendCode(data.updatedArtifact)
        }
      }
    } catch (error) {
      ...
      <div className="min-h-screen flex items-center justify-center text-text-secondary">
        <div className="text-center">
          <div className="inline-flex items-center justify-center gap-2 mb-4">
            ...
            <span>Loading frontend code...</span>
          </div>
        </div>
      </div>
    )
  ...
  if (frontendCode.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center text-text-secondary">
        <div className="text-center">
          <p>No frontend code found. Please return to the workflow.</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-text-primary">Frontend Code Review</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[65%_35%] gap-6 max-w-7xl mx-auto">
          ...
          <div className="glass-card p-6 border border-border rounded-2xl">
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-text-primary mb-2">Generated Frontend Code</h2>
              <p className="text-sm text-text-secondary">
                Review the React/Next.js components generated based on the wireframes
              </p>
            </div>

            <div className="h-[600px]">
              <CodeViewer files={frontendCode} />
            </div>
              ...
              <div className="glass-card p-4 border border-border rounded-lg">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold text-text-primary">{frontendCode.length}</span>
                  </div>
...
export default function FrontendCodePage() {
  return (
